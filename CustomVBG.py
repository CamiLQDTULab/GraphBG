""" Optimized Customized Bayesian Gaussian Mixture Model """
...
class CustomBayesianGaussianMixture(BaseMixture):
    def __init__(
        self,
        *,
        n_components=1,
    ...
    def _check_weights_parameters(self):
        self.weight_concentration_prior_ = (
            1.0 / self.n_components if self.weight_concentration_prior is None else self.weight_concentration_prior
        )

    def _check_means_parameters(self, X):
        _, n_features = X.shape
        self.mean_precision_prior_ = 1.0 if self.mean_precision_prior is None else self.mean_precision_prior
        self.mean_prior_ = X.mean(axis=0) if self.mean_prior is None else check_array(
    ...
    def _check_precision_parameters(self, X):
        ...
        if self.degrees_of_freedom_prior is None:
            ...
            raise ValueError(
                f"The parameter 'degrees_of_freedom_prior' should be > {n_features - 1}, got {self.degrees_of_freedom_prior}"
            )
    ...
    def _checkcovariance_prior_parameter(self, X):
        ...
        if self.covariance_prior is None:
            ...
            cov_prior = check_array(self.covariance_prior, dtype=[np.float64, np.float32], ensure_2d=False)
            self.covariance_prior_ = cov_prior
            if self.covariance_type in ["full", "tied"]:
                _check_shape(cov_prior, (n_features, n_features), f"{self.covariance_type} covariance_prior")
                _check_precision_matrix(cov_prior, self.covariance_type)
            elif self.covariance_type == "diag":
                _check_shape(cov_prior, (n_features,), f"{self.covariance_type} covariance_prior")
                _check_precision_positivity(cov_prior, self.covariance_type)
    ...
    def _estimate_wishart_full(self, nk, xk, sk):
        ...
        outer = (
            ...
            * self.mean_precision_prior_
            / self.mean_precision_[:, None, None]
            * np.einsum("ni,nj->nij", diff, diff)
        )
        self.covariances_ = (
            self.covariance_prior_ + nk[:, None, None] * sk + outer
        ) / self.degrees_of_freedom_[:, None, None]
    ...
    def _estimate_wishart_diag(self, nk, xk, sk):
        diff = xk - self.mean_prior_
        self.degrees_of_freedom_ = self.degrees_of_freedom_prior_ + nk
        self.covariances_ = self.covariance_prior_ + nk[:, None] * (
            sk + (self.mean_precision_prior_ / self.mean_precision_)[:, None] * diff**2
        )
    ...
    def _estimate_wishart_spherical(self, nk, xk, sk):
        ...
        self.covariances_ = self.covariance_prior_ + nk * (
            sk + self.mean_precision_prior_ / self.mean_precision_ * np.mean(diff**2, axis=1)
        )
    ...
    def _estimate_log_prob(self, X):
        ...
        if not hasattr(self, "_feature_index"):
            self._feature_index = np.arange(n_features)[:, None]
        log_lambda = n_features * np.log(2.0) + np.sum(
            digamma(0.5 * (self.degrees_of_freedom_ - self._feature_index)), 0
        )
        return log_gauss + 0.5 * (log_lambda - n_features / self.mean_precision_)

    def _compute_lower_bound(self, log_resp, log_prob_norm):
        n_features = self.mean_prior_.shape[0]
        log_det_prec = _compute_log_det_cholesky(
            self.precisions_cholesky_, self.covariance_type, n_features
        ) - 0.5 * n_features * np.log(self.degrees_of_freedom_)

        if self.covariance_type == "tied":
            log_wishart = self.n_components * float(
                _log_wishart_norm(self.degrees_of_freedom_, log_det_prec, n_features)
            )
        else:
            log_wishart = np.sum(_log_wishart_norm(self.degrees_of_freedom_, log_det_prec, n_features))

        log_norm_weight = (
            -np.sum(betaln(self.weight_concentration_[0], self.weight_concentration_[1]))
            if self.weight_concentration_prior_type == "dirichlet_process"
            else _log_dirichlet_norm(self.weight_concentration_)
        )
    ...
    def _set_parameters(self, params):
        ...
        if self.weight_concentration_prior_type == "dirichlet_process":
            sum_ab = self.weight_concentration_[0] + self.weight_concentration_[1]
            tmp = self.weight_concentration_[1] / sum_ab
            self.weights_ = (
                self.weight_concentration_[0] / sum_ab * np.hstack((1, np.cumprod(tmp[:-1])))
            )
        ...
        if self.covariance_type == "full":
            self.precisions_ = np.array(
                [pc @ pc.T for pc in self.precisions_cholesky_]
            )
